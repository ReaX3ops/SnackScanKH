...
.auth-icon {{ font-size: 2.5rem; margin-bottom: 0.6rem; }}
.auth-title {{
    font-size: 1.75rem;
    font-weight: 800;
    color: #111;
...
.auth-sub {{ font-size: 0.86rem; color: #999; margin: 0 0 1.6rem; }}
...
if st.session_state.user:
    ...
    <div class="auth-card">
        ...
        <div class="auth-title">You're in!</div>
        <div class="auth-sub">Signed in as <b>{st.session_state.user['email']}</b></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("Sign In →", key="signin_btn"):
        if not email or not password:
            ...
            try:
                ...
                st.markdown('<div class="success-box">✅ Signed in! Redirecting...</div>', unsafe_allow_html=True)
                st.switch_page("app.py")
            except Exception:
    ...
    if st.button("Create Account →", key="signup_btn"):
        if not new_email or not new_password or not confirm_pass:
            ...
            try:
                ...
                st.markdown('<div class="success-box">🎉 Account created! Welcome to SnackScanKH!</div>', unsafe_allow_html=True)
                st.switch_page("app.py")
            except Exception:
